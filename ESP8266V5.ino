...
#define DELAY_TO_REACH_MAX_SERVO_POS 1000
#define DELAY_TO_REACH_NEXT_XY_POS 1000
#define DELAY_TO_REACH_NEXT_CLOSE_POS 400

#define SERVO_COUNT 8
#define INSTRUCTIONS_COUNT 20
#define INSTRUCTIONS_WIDTH 30
#define DEBUG 0
...
int my_sequence[INSTRUCTIONS_COUNT][INSTRUCTIONS_WIDTH];  // déclarer un tableau de tableaux pour stocker les données du fichier
...
bool handleSelect(String path) { // send the right file to the client (if it exists)
  if (DEBUG) Serial.println("handleSelect: " + path);
  if (DEBUG) Serial.println(server.argName(0));
  ...
  wFile.println("<body>");
  wFile.println("<form method='GET'");
 Dir dir = SPIFFS.openDir("/");
  // or Dir dir = LittleFS.openDir("/data");
  while (dir.next()) {
    String namef = dir.fileName();
    if (DEBUG) Serial.print(namef);
    if(namef.indexOf(".txt")>0){
      String sr = "<input type='submit' name='action'  value='";
      String endl = "'>";
      wFile.println(sr+namef+endl);
    }
}
...
void handleSelectPOST() {  // upload a new file to the SPIFFS
  if (DEBUG) Serial.println("handle select");
  if (DEBUG) Serial.println(server.argName(0));
...
void setup_server() {
  ...
  server.on("/select", HTTP_GET, []() {                  // if the client requests the upload page
    if (!handleSelect("/select.html"))                 // send it if it exists
      server.send(404, "text/plain", "404: Not Found");  // otherwise, respond with a 404 (Not Found) error
  });
...
void setup() {
  ...
  // or Dir dir = LittleFS.openDir("/data");
  while (dir.next()) {
    if (DEBUG) Serial.print(dir.fileName());
    if(dir.fileSize()) {
...
void loop() {
//if (DEBUG) Serial.println("handle wm");
//  WiFiManager.process();
    
  server.handleClient();
  MDNS.update();

  boolean atLeastOneServoMove = false;
  ...
  if (launchSequence) {
    ...
    for (int i = 0; i < INSTRUCTIONS_COUNT; i++) {
      ...
      for(int scale = 0; scale < 7; scale++){
        //if (scale < 3){
          for(int j=0; j < SERVO_COUNT; j++){
            ...
            if(my_new_sequence[j] > IGNORE_THRESHOLD) atLeastOneServoMove = true;
          }
       ...
       /* else{
        ...
        if(atLeastOneServoMove){
            //////////////////////////////////////////////////////////////delay(1000);
            move_servos(my_new_sequence);
            delay(DELAY_TO_REACH_MAX_SERVO_POS);
            move_servos(neutral_position);
            delay(DELAY_TO_REACH_MAX_SERVO_POS);
        }
        else{
        ...
        if (scale != 6){
          Serial.write("G1 X0.133 Y0 F200 \n");
          delay(DELAY_TO_REACH_NEXT_CLOSE_POS);
        }
      ...
      //delay(DELAY_TO_REACH_NEXT_XY_POS);
      
      Serial.write("G1 X-0.798 Y0 F200 \n");
      Serial.write("G1 X0 Z0.222 F200 \n");
      delay(DELAY_TO_REACH_NEXT_XY_POS);
